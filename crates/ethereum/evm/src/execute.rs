...
use revm_primitives::{
    ...
    BlockEnv, CfgEnvWithHandlerCfg, ChainAddress, EnvWithHandlerCfg, ResultAndState, U256,
};
...
{
    type Strategy<DB: SyncDatabase<Error: Into<ProviderError> + Display>> =
    ...
    EvmConfig: ConfigureEvm<Header = alloy_consensus::Header>,
{
    ...
    fn apply_post_execution_changes(
        ...
        // increment balances
        self.state
            .increment_balances(balance_increments.iter().map(|(a, v)| (ChainAddress(self.chain_spec.chain_id(), *a), *v)))
            .map_err(|_| BlockValidationError::IncrementBalanceFailed)?;
...
mod tests {
    ...
    fn eip_4788_empty_account_call() {
        ...
        let nonce =
            executor.with_state_mut(|state| state.basic(ChainAddress(chain_id, SYSTEM_ADDRESS)).unwrap().unwrap().nonce);
        assert_eq!(nonce, 0);
    }
    ...
    fn eip_4788_high_base_fee() {
        ...
        let timestamp_storage = executor.with_state_mut(|state| {
            state.storage(ChainAddress(chain_id, BEACON_ROOTS_ADDRESS), U256::from(timestamp_index)).unwrap()
        });
        assert_eq!(timestamp_storage, U256::from(header.timestamp));

        // get parent beacon block root storage and compare
        let parent_beacon_block_root_storage = executor.with_state_mut(|state| {
            state.storage(ChainAddress(chain_id, BEACON_ROOTS_ADDRESS), U256::from(parent_beacon_block_root_index)).unwrap()
        });
        assert_eq!(parent_beacon_block_root_storage, U256::from(0x69));
    }
    ...
    fn eip_2935_pre_fork() {
        ...
        // loaded
        executor.with_state_mut(|state| state.basic(ChainAddress(chain_id, HISTORY_STORAGE_ADDRESS)).unwrap());
    ...
    fn eip_2935_fork_activation_within_window_bounds() {
        ...
        assert!(executor
            .with_state_mut(|state| state.basic(ChainAddress(chain_id, HISTORY_STORAGE_ADDRESS)).unwrap().is_some()));
        assert_ne!(
