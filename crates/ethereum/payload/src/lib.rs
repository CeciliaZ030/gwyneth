    ...
    F: FnOnce(BestTransactionsAttributes) -> BestTransactionsIter<Pool>,
{
    ...
    let mut db =
        State::builder().with_database(sync_cached_reads.as_db_mut(state)).with_bundle_update().build();
    ...
    let WithdrawalsOutcome { withdrawals_root, withdrawals } =
        commit_withdrawals(chain_spec.chain.id(), &mut db, &chain_spec, attributes.timestamp, attributes.withdrawals)?;
    ...
    let (state_root, trie_output) = {
        db.database.db.get_db(chain_spec.chain.id()).unwrap().state_root_with_updates(hashed_state.clone()).inspect_err(|err| {
            warn!(target: "payload_builder",
                parent_hash=%parent_header.hash(),
                %err,
