...
#[inline]
pub fn default_gwyneth_payload<EvmConfig, Pool, Client, SyncProvider>(
    ...
    SyncProvider: StateProvider,
{
    ...
    let (state_root, trie_output) = {
        sync_db.database.db.get_db(chain_spec.chain.id()).unwrap().state_root_with_updates(hashed_state.clone()).inspect_err(|err| {
            warn!(target: "payload_builder",
                parent_hash=%parent_header.hash(),
                %err,
