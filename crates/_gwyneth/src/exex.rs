...
type GwynethFullNode1 = FullNode<  
    NodeAdapter<FullNodeTypesAdapter<GwynethNode, Arc<DatabaseEnv>, BlockchainProvider<Arc<DatabaseEnv>>>,  
    Components<  
...
type GwynethFullNode2 = FullNode<  
    NodeAdapter<FullNodeTypesAdapter<GwynethNode, Arc<DatabaseEnv>, BlockchainProvider2<Arc<DatabaseEnv>>>,  
    Components<  
        FullNodeTypesAdapter<  
            GwynethNode,
            Arc<DatabaseEnv>,  
    ...
    pub async fn start(mut self) -> eyre::Result<()> {
        while let Some(notification) = self.ctx.notifications.next().await {
            ...
            if let Some(committed_chain) = notification.committed_chain() {
                ...
                let height = BlockNumHash::new(committed_chain.tip().number, committed_chain.tip().hash());
